...
class TestGenex_database:
    ...
    def test_from_csv(self):
        ...
        test_db = gxdb.from_csv(file_name=data_file, sc=self.sc, feature_num=feature_num, is_header=False)
        ...
        test_db = gxdb.from_csv(file_name=data_file, sc=self.sc, feature_num=feature_num)
        test_db_id_ls = [x[0] for x in test_db.data]
    ...
    def test_from_db(self):
        ...
        db = gxdb.from_csv(data_file, sc=self.sc, feature_num=feature_num, is_header=False)
        db_attribute = db.conf
        db_data = db.data
        db.save(path=path)

        db_after_save = gxdb.from_db(self.sc, path)
    ...
    def test_build_cluster(self):
        ...
        mydb = gxdb.from_csv(data_file, sc=self.sc, feature_num=feature_num, is_header=False)
